...
export default function NestedExperienceItem({
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg border border-gray-200/10 p-4 sm:p-8 shadow-sm/10">
        ...
        >
          <div className="flex gap-4">
            ...
            <div className="flex-1 min-w-0 mb-4">
              <h2 className="font-semibold text-gray-900 text-base break-words">
                {title}
              </h2>
              <p className="font-sans text-xs text-gray-700 break-words mt-1">
                {experiences.length} position
                {experiences.length > 1 ? "s" : ""} · {totalDuration}
              </p>
              <p className="font-sans text-xs text-gray-600 break-words mt-1">
        ...
        <div className={`hidden sm:block ${!isExpanded ? "pb-8" : ""}`}>
          <div className={`flex gap-4 ${isExpanded ? "mb-6" : "mb-0"}`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="font-sans text-sm text-gray-700 break-words mt-1">
                {experiences.length} position
                {experiences.length > 1 ? "s" : ""} · {totalDuration}
              </p>
              <p className="font-sans text-xs text-gray-600 break-words mt-1">
          ...
          {!isExpanded && (
            <div className="flex flex-wrap items-center gap-1 ml-16 mt-2">
              {combinedSkills.slice(0, 5).map((tech, index) => (
                <span
                  key={tech}
                  className="font-sans text-xs font-medium text-gray-600 px-2 py-0.5 bg-gray-100 rounded-full whitespace-nowrap"
                  style={{
              ...
              {combinedSkills.length > 5 && (
                <span className="font-sans text-xs font-medium text-gray-500 px-2 py-0.5 bg-gray-100 rounded-full whitespace-nowrap">
              ...
              >
                ...
                <div className="flex gap-4 mb-4">
                  ...
                  <div className="flex-1 min-w-0 space-y-0.5">
                    ...
                    <p className="font-sans text-xs text-gray-700 break-words">
                ...
                <div className="sm:ml-14 space-y-4">
                  <p className="text-pretty font-sans text-sm leading-relaxed text-gray-900 break-words">
                    {experience.description}
                  </p>
                  <ul className="list-disc pl-5 text-pretty font-sans text-sm leading-relaxed text-gray-900 space-y-2">
                    {experience.achievements.map(
                      (achievement, achievementIndex) => (
                        <li key={achievementIndex} className="break-words">
      ...
      {isExpanded && (
        <div
          className={`mt-8 space-y-4 border-t border-gray-200 pt-8 sm:ml-6 transition-all duration-700 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"
          }`}
