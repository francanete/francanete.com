...
export default function NestedExperienceItem({ experiences, title }: NestedExperienceItemProps) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-lg border border-gray-200/10 p-8 shadow-sm/10">
        ...
        <div className={`flex gap-4 ${isExpanded ? 'mb-6' : 'mb-0'}`}>
          ...
          <div className="flex-1 space-y-0.5">
            ...
            <p className="font-sans text-sm text-gray-700">
              {experiences.length} position{experiences.length > 1 ? 's' : ''} · {totalDuration}
            </p>
            <div className="flex flex-wrap items-center text-xs text-gray-600">
              <p>{experiences[0].location} · {experiences[0].locationType}</p>
            </div>
          </div>
        ...
        >
          <div className="space-y-6 ml-6">
            {experiences.map((experience, index) => (
              <div key={experience.id} className="border-t border-gray-200 pt-6 first:border-t-0 first:pt-0">
                ...
                <div className="flex gap-4 mb-4">
                  ...
                  <div className="flex-1 space-y-0.5">
                    ...
                    <div className="flex flex-wrap items-center text-xs text-gray-600">
                      <p>{formatDateRange(experience.startDate, experience.endDate)} · {calculateDuration(experience.startDate, experience.endDate)} · {experience.location} · {experience.locationType}</p>
                    </div>
                  </div>
                </div>
      ...
      {isExpanded && (
        <div
          className={`mt-8 space-y-4 border-t border-gray-200 pt-8 transition-all duration-700 ${
            isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'
          }`}
